...
service = Service.create([
  { service_name: "Wedding Photography", description: "We offer different packages for Wedding Photography. Capture your special day moments with us! After all, Wedding is the special event in anyone's life.", status: 1, service_icon_id:  service_icons[0].id, user_id: user.id },
  { service_name: "Pre-Wedding Photography", description: "We provide themed Pre-Wedding photography & videography. We capture and bring emotion in the motions of Life.  Make your pre-wedding days memorable!", status: 1, service_icon_id:  service_icons[1].id, user_id: user.id },
  { service_name: "Candid Photography", description: "We love clicking Candid Photographs, of the unexpected moments. We capture your Smile, Love and Joy with light composition of Candid Shots. It's about capturing Perfect Moment. ", status: 1, service_icon_id:  service_icons[2].id, user_id: user.id },
  { service_name: "Model Shoot", description: "We are highly instrumental in offering Modelling photography services to the clients. our services includes Fashion Photography, Modelling Photo Shoot and Modelling Services.", status: 1, service_icon_id:  service_icons[3].id, user_id: user.id },
  { service_name: "Couple Photography", description: "We are expert in Couple Shoot During the Couple Shoot, we create a Story for you and provide Props. Contact us to get a memorable gift for you and your partner in future!", status: 1, service_icon_id:  service_icons[4].id, user_id: user.id },
  { service_name: "Kids Photography", description: "We undertake indoor and outdoor Kids Photo Shoots. Families have preferred this Kids Photography to have the whole time with lasting memories and also to fill walls within framed professional stills. ", status: 1, service_icon_id:  service_icons[5].id, user_id: user.id }
])
...
testimonials = Testimonial.create!([
  ...
  { client_name: "Anjali Chauhan", message: "I want to say HUGE thank you to Sagar Gadani for capturing beautiful pictures of our pre-wedding photo shoot. Our pre-wedding photographs were beyond perfect. You have amazing creative skills. The photos are absolutely beautiful, you managed to capture our special day in such an amazing way. Every time we look at the images we can recall all the emotion, the laughs and fun of the day. Thanks again.", status: 'active', user_id: user.id },
  { client_name: "Apurva Chauhan", message: "The best thing of your photography is how you conversate with your camera while clicking. You have an immense hold on poses, design, edition, crafting and ofcourse creativity in any occasion whatever you are brought forth for. Good Luck.", status: 'active', user_id: user.id },
  { client_name: "Vaibhav Prakash", message: "Good Work and More Creativity you bring to the Photography. Thank you and I wish you a good luck.", status: 'active', user_id: user.id }
])
